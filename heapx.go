...
// Enables O(log n) updates via Fix, no heap.Interface boilerplate needed
//
...
func New[V any](vLess func(a, b V) bool) *Heap[V] {
	h := &Heap[V]{
		data: newRawHeap(vLess),
	}
	heap.Init(h.data)
	return h
}

// Push adds a value to the heap and returns its node (for subsequent Fix/Remove)
//
